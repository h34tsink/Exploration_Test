...
function initTrace() {
    ...
    btn.addEventListener('click', () => {
        ...
        setTimeout(() => {
            const target = signals.reduce((acc, s) => ({
                x: acc.x + s.x / signals.length,
                y: acc.y + s.y / signals.length
            }), { x: 0, y: 0 });

            const distance = Math.sqrt(Math.pow(target.x - playerPos.x, 2) + Math.pow(target.y - playerPos.y, 2));
...
function drawTrace(ctx, canvas, signals, playerPos) {
    ...
    signals.forEach(signal => {
        const gradient = ctx.createRadialGradient(signal.x, signal.y, 0, signal.x, signal.y, signal.strength);
...
function initSample() {
    ...
    const targetsContainer = document.querySelector('[data-verb="sample"] .sample-targets');

    // Create sample targets
    const targets = Array(6).fill(null).map((_, i) => {
    ...
    targets.forEach(target => {
        target.addEventListener('click', () => {
            if (!target.classList.contains('sampled')) {
                ...
                const sampleTypes = ['Enzyme', 'Mineral', 'Organic', 'Crystal', 'Gas', 'Fluid'];
                const sample = sampleTypes[Math.floor(Math.random() * sampleTypes.length)];
                ...
                resultBox.innerHTML = `
                    <div class="reward-item">
                        ...
                        <p class="rep-gain">+${Math.floor(Math.random() * 30 + 20)} REP</p>
                    </div>
                `;
            }
        });
...
function initDecode() {
    ...
    const patternDisplay = document.querySelector('[data-verb="decode"] .pattern-display');
    const input = document.querySelector('[data-verb="decode"] .pattern-input');

    const patterns = [
        { pattern: '⬡ ⬢ ⬡ ⬢ ⬡', answer: 'hexagon', reward: 'Propulsion Blueprint Shard' },
        { pattern: '△ ▽ △ ▽ △', answer: 'triangle', reward: 'Energy Blueprint Shard' },
        { pattern: '◯ ◉ ◯ ◉ ◯', answer: 'circle', reward: 'Shield Blueprint Shard' },
        ...
        if (userAnswer === currentPattern.answer) {
            resultBox.innerHTML = `
                <div class="reward-item">
                    ...
                    <p class="rep-gain">+${Math.floor(Math.random() * 50 + 50)} REP</p>
                </div>
            `;
...
function initShadow() {
    ...
    const trackingDisplay = document.querySelector('[data-verb="shadow"] .tracking-display');
            ...
            if (step < path.length) {
                ...
                resultBox.innerHTML = `
                    <div class="reward-item">
                        ...
                        <p class="rep-gain">+${Math.floor(Math.random() * 40 + 30)} REP</p>
                    </div>
                `;
            }
        }, 1000);
...
function initExcavate() {
    ...
    const resultBox = document.querySelector('[data-verb="excavate"] .result-box');
    const progressBar = document.querySelector('[data-verb="excavate"] .progress-bar');
    ...
    btn.addEventListener('click', () => {
        ...
        const interval = setInterval(() => {
            progress += 10;
            progressBar.querySelector('::after') || (progressBar.style.background = `linear-gradient(90deg, var(--primary) ${progress}%, rgba(0,0,0,0.3) ${progress}%)`);
            
            if (progress >= 100) {
                clearInterval(interval);
                const artifacts = ['Ancient Relic', 'Precursor Data Cube', 'Crystalline Artifact', 'Quantum Fragment'];
                ...
                resultBox.innerHTML = `
                    <div class="reward-item">
                        ...
                        <p>Condition: ${(Math.random() * 30 + 70).toFixed(1)}%</p>
                        <p class="rep-gain">+${Math.floor(Math.random() * 60 + 40)} REP</p>
                    </div>
                `;
            }
        }, 200);
...
function initMaterialGenerator() {
    ...
    document.getElementById('calculateResearch')?.addEventListener('click', () => calculateResearch());
...
function generateBatchMaterials() {
    for (let i = 0; i < 10; i++) {
        const types = ['plant', 'animal', 'mineral', 'crystal', 'gas', 'artifact', 'microorganism', 'compound'];
        const biomes = ['volcanic', 'arctic', 'forest', 'desert', 'ocean', 'cave', 'asteroid', 'space', 'radioactive'];
...
function generateMaterial(type, biome, rarityOverride = 'random') {
    ...
    const prefixes = {
        ...
        radioactive: ['Radio', 'Quantum', 'Atomic', 'Isotopic', 'Nuclear', 'Plutonic']
    };
    ...
    const suffixes = {
        ...
        artifact: ['relic', 'artifact', 'construct', 'remnant', 'monolith', 'cache'],
        microorganism: ['bacteria', 'microbe', 'spore', 'colony', 'culture', 'strain'],
        compound: ['compound', 'mixture', 'solution', 'catalyst', 'reagent', 'polymer']
    };
    ...
    const repValue = Math.floor(baseREP[rarity] * (1 + Object.values(fieldTotals).reduce((a, b) => a + b, 0) / 1000));
...
function displayMaterial(material, container) {
    ...
    container.innerHTML = `
        <div class="material-name">
            ...
            <span style="color: ${rarityColors[material.rarity]}">${material.name}</span>
        </div>
        ...
        <div style="display: flex; gap: 10px; margin-bottom: 15px; flex-wrap: wrap;">
            ...
            <span class="material-type-badge" style="background: ${rarityColors[material.rarity]}; color: #fff;">
        ...
        <div class="material-stats">
            ...
            ${topFields.map(([field, total]) => `
                <div class="stat-row">
                    <span class="stat-label">${SCIENTIFIC_FIELDS[field].name}:</span>
                    <span class="stat-value">${total.toFixed(0)}%</span>
                </div>
                <div class="stat-bar-container">
                    <div class="stat-bar-fill" style="width: ${Math.min(100, total / 4)}%; background: ${SCIENTIFIC_FIELDS[field].color};"></div>
                </div>
            `).join('')}
            
            <div class="stat-row" style="margin-top: 15px;">
                <span class="stat-label">Total Stat Pool:</span>
                <span class="stat-value">${Object.values(material.fieldTotals).reduce((a, b) => a + b, 0).toFixed(0)}</span>
            </div>
        ...
        <div style="margin-top: 15px; padding: 10px; background: rgba(0,0,0,0.3); border-radius: 5px; font-size: 0.85em;">
            <strong>Origin:</strong> ${material.biome.charAt(0).toUpperCase() + material.biome.slice(1)}<br>
            <strong>Discovered:</strong> ${new Date(material.discovered).toLocaleTimeString()}<br>
            ${material.evolved ? `<strong style="color: #e67e22;">⚡ Evolved (${material.mutations.length} mutations)</strong>` : ''}
        </div>
    `;
}
...
function displayScientificStats(material) {
    ...
    statsDiv.innerHTML = Object.keys(SCIENTIFIC_FIELDS).map(fieldKey => {
        ...
        return `
            <div class="field-card">
                ...
                <div class="field-total" title="Sum of 4 subfields below">${total.toFixed(0)}% total</div>
                ${field.subfields.map(subfield => `
                    <div class="subfield-stat">
                        <span class="subfield-name">${subfield}:</span>
                        <span class="subfield-value">${material.stats[fieldKey][subfield].toFixed(0)}%</span>
                    </div>
                `).join('')}
            </div>
        `;
...
function evolveMaterial() {
    ...
    if (!['plant', 'animal', 'microorganism'].includes(state.currentMaterial.type)) {
    ...
    const subfieldKey = SCIENTIFIC_FIELDS[randomField].subfields[
        Math.floor(Math.random() * SCIENTIFIC_FIELDS[randomField].subfields.length)
    ];
    
    const boost = Math.floor(Math.random() * 15 + 10); // 10-25% boost
    state.currentMaterial.stats[randomField][subfieldKey] = 
        Math.min(100, state.currentMaterial.stats[randomField][subfieldKey] + boost);
    
    // Recalculate totals
    Object.keys(state.currentMaterial.stats).forEach(field => {
        state.currentMaterial.fieldTotals[field] = 
            Object.values(state.currentMaterial.stats[field]).reduce((a, b) => a + b, 0);
    });
    ...
    displayMaterial(state.currentMaterial, document.getElementById('currentMaterial'));
    ...
    document.getElementById('materialAnalysis').innerHTML = `
        <div style="color: #e67e22; font-weight: bold;">
            ...
            <span style="color: ${SCIENTIFIC_FIELDS[randomField].color}">${SCIENTIFIC_FIELDS[randomField].name}</span> 
            → ${subfieldKey}: +${boost}%<br>
            New total: ${state.currentMaterial.fieldTotals[randomField].toFixed(0)}%
        </div>
    `;
}
...
function analyzeMaterial() {
    ...
    const topSubfields = allSubfields.sort((a, b) => b.value - a.value).slice(0, 5);
    ...
    Object.entries(TECH_TREE).forEach(([branch, branchData]) => {
        Object.entries(branchData.techs).forEach(([techKey, tech]) => {
            const meetsReqs = Object.entries(tech.requirements).every(([field, required]) => {
    ...
    analysisDiv.innerHTML = `
        <h4 style="color: var(--primary); margin-bottom: 10px;">Analysis Report: ${m.name}</h4>
        
        <div style="margin-bottom: 15px;">
            <strong>Strongest Fields:</strong>
            <div style="margin-top: 5px;">
                ${sortedFields.map(([field, total]) => `
                    <div style="margin: 5px 0;">
                        <span style="color: ${SCIENTIFIC_FIELDS[field].color}">■</span>
                        ${SCIENTIFIC_FIELDS[field].name}: <strong>${total.toFixed(0)}%</strong>
                    </div>
                `).join('')}
            </div>
        </div>
        ...
        <div style="margin-top: 10px; padding: 10px; background: rgba(46, 204, 113, 0.1); border-left: 3px solid #2ecc71;">
            <strong>Research Value:</strong> This ${m.rarity} ${m.type} is worth <span class="rep-gain">+${m.repValue} REP</span> when cataloged.
            ${m.rarity === 'epic' || m.rarity === 'legendary' ? '<br><strong style="color: #f39c12;">⭐ First discoverer bonus available!</strong>' : ''}
        </div>
    `;
}
...
function updateInventory() {
    ...
    inventoryDiv.innerHTML = filtered.map(m => {
        const typeIcons = {
            plant: '🌱',
            animal: '�',
            mineral: '💎',
        ...
        const topField = Object.entries(m.fieldTotals).sort((a, b) => b[1] - a[1])[0];
        
        return `
            <div class="material-card" onclick="selectMaterial(${m.id})">
                <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 10px;">
                    <span style="font-weight: bold; color: ${rarityColors[m.rarity]}">
                        ${typeIcons[m.type]} ${m.name}
                    </span>
                    <span style="font-size: 0.8em; color: #95a5a6;">${m.type}</span>
                </div>
                <div style="display: flex; justify-content: space-between; font-size: 0.85em; align-items: center;">
                    <span style="color: ${SCIENTIFIC_FIELDS[topField[0]].color}">
                        ${SCIENTIFIC_FIELDS[topField[0]].name}: ${topField[1].toFixed(0)}%
                    </span>
                    <span class="rep-gain" style="font-size: 0.9em;">+${m.repValue} REP</span>
                </div>
            </div>
        `;
    }).join('');
...
function filterMaterialsByType(materials, filterType) {
    switch (filterType) {
        ...
        case 'rare':
            return materials.filter(m => m.rarity === 'rare' || m.rarity === 'epic' || m.rarity === 'legendary');
        default:
            return materials;
    }
}
...
function displayTechRequirements(branch) {
    ...
    techDiv.innerHTML = `
        ...
        ${Object.entries(branchData.techs).map(([key, tech]) => {
            const requirements = Object.entries(tech.requirements);
            const allMet = requirements.every(([field, required]) => 
                availableStats[field] >= required
            );
            
            let subfieldStatus = '';
            if (tech.subfieldReqs) {
                const subfieldsMet = Object.entries(tech.subfieldReqs).map(([subfield, required]) => {
                ...
                subfieldStatus = `
                    <div style="margin-top: 10px; padding-top: 10px; border-top: 1px solid var(--border);">
                        ...
                        ${subfieldsMet.map(sf => {
                            const percentage = sf.required > 0 ? ((sf.available / sf.required) * 100).toFixed(0) : 0;
                            return `
                                <div class="requirement-item ${sf.met ? 'requirement-met' : 'requirement-unmet'}">
                                    <span>${sf.subfield}:</span>
                                    <span>${percentage}% ${sf.met ? '✓' : '✗'}</span>
                                </div>
                            `;
                        }).join('')}
                    </div>
                ...
                <div class="tech-card ${allMet ? 'unlockable' : 'locked'}">
                    ...
                    <div class="tech-requirements-list">
                        ${requirements.map(([field, required]) => {
                            ...
                            const met = available >= required;
                            const percentage = required > 0 ? ((available / required) * 100).toFixed(0) : 0;
                            return `
                                <div class="requirement-item ${met ? 'requirement-met' : 'requirement-unmet'}">
                                    <span style="color: ${SCIENTIFIC_FIELDS[field].color}">
...
function calculateResearch() {
    ...
    Object.entries(TECH_TREE).forEach(([branch, branchData]) => {
        Object.entries(branchData.techs).forEach(([key, tech]) => {
            ...
            if (tech.subfieldReqs) {
                meetsSubfields = Object.entries(tech.subfieldReqs).every(([subfield, required]) => {
    ...
    resultsDiv.innerHTML = `
        <div class="research-results">
            ...
            <div style="margin-bottom: 10px;">
                <strong>Total Objects Allocated:</strong> ${state.materials.length}
            </div>
            <div style="margin-bottom: 10px;">
            ...
            ${unlockedTechs.length > 0 ? `
                <div style="margin-top: 10px; padding: 10px; background: rgba(46, 204, 113, 0.2); border-radius: 5px;">
                    ${unlockedTechs.map(tech => `<div>✓ ${tech}</div>`).join('')}
                </div>
            ` : `
...
function initRewardSystem() {
    ...
    if (clearBtn) {
        clearBtn.addEventListener('click', () => {
            state.rewards = [];
            updateRewardStats();
            document.getElementById('discoveryLog').innerHTML = '<p style="color: #95a5a6; font-style: italic;">No discoveries yet. Start generating rewards above.</p>';
        });
    }
...
function generateReward(rarity, globalCount) {
    const rewards = {
        common: { base: 20, types: ['Blueprint Shard', 'Catalyst Sample', 'Data Fragment'] },
        uncommon: { base: 50, types: ['Refined Catalyst', 'Complete Blueprint', 'Specimen Card'] },
        rare: { base: 100, types: ['Artifact', 'Advanced Blueprint', 'Data Core'] },
        unique: { base: 250, types: ['Ancient Relic', 'Legendary Blueprint', 'Precursor Artifact'] }
    };

    const rewardData = rewards[rarity];
    const type = rewardData.types[Math.floor(Math.random() * rewardData.types.length)];
    ...
    const repGain = Math.floor(rewardData.base * firstDiscovererBonus * rarityMultiplier);
...
function displayReward(reward, container) {
    ...
    container.innerHTML = `
        <div class="reward-item" style="border: 2px solid ${rarityColors[reward.rarity]}; background: rgba(0,0,0,0.3); padding: 15px; border-radius: 8px; margin-top: 15px;">
            <h4 style="color: ${rarityColors[reward.rarity]}; margin-top: 0;">${reward.type}</h4>
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 10px; font-size: 0.9em;">
                ...
                <div><strong>Multiplier:</strong> ${(reward.multiplier * 100).toFixed(0)}%</div>
            </div>
            ${reward.isFirstDiscoverer ? '<p style="color: #f39c12; margin: 10px 0; font-weight: bold;">⭐ FIRST DISCOVERER BONUS! (2x REP)</p>' : ''}
            <div style="text-align: center; margin-top: 15px; padding: 15px; background: rgba(46, 204, 113, 0.2); border-radius: 5px;">
                ...
                <div class="rep-gain" style="font-size: 2em; color: #2ecc71; font-weight: bold;">+${reward.repGain}</div>
            </div>
        </div>
    `;
}
...
function displayComparison(reward) {
    ...
    comparisonDiv.innerHTML = `
        <div style="font-size: 0.9em; margin-bottom: 10px; color: #95a5a6;">
            REP value for <strong style="color: ${rarityColors[reward.rarity]}">${reward.rarity.toUpperCase()}</strong> discoveries:
        </div>
        ${comparisons.map(c => `
...
function updateRewardStats() {
    ...
    const avgREP = state.rewards.length > 0 ? Math.floor(totalREP / state.rewards.length) : 0;
...
function logDiscovery(reward) {
    ...
    entry.style.cssText = 'padding: 8px; margin-bottom: 5px; background: rgba(0,0,0,0.3); border-radius: 5px; border-left: 3px solid ' + rarityColors[reward.rarity];
    entry.innerHTML = `
        <div style="display: flex; justify-content: space-between; align-items: center;">
            <span style="color: ${rarityColors[reward.rarity]}; font-weight: bold;">${reward.type}</span>
...
function advanceTime() {
    ...
    const tempVariation = Math.sin((state.ecology.timeOfDay / 24) * Math.PI * 2) * 15;
    ...
    if (state.ecology.timeOfDay >= 18 || state.ecology.timeOfDay <= 6) {
        state.ecology.faunaActivity = Math.min(100, state.ecology.faunaActivity + 5);
    } else {
    ...
    if (state.ecology.floraDensity < 100) {
        state.ecology.floraDensity = Math.min(100, state.ecology.floraDensity + 2);
        state.ecology.worldHealth = Math.min(100, state.ecology.worldHealth + 1);
    }
    
    addEcologyLog(`Time advanced to ${state.ecology.timeOfDay}:00. Temperature: ${state.ecology.temperature.toFixed(1)}°C`);
}

function harvestResource() {
    state.ecology.harvestCount++;
    state.ecology.floraDensity = Math.max(0, state.ecology.floraDensity - 15);
    ...
    addEcologyLog(`Resource harvested. Flora density decreased. Health: ${state.ecology.worldHealth}%`);
}
...
function drawEcology(ctx, canvas) {
    ...
    const isDaytime = state.ecology.timeOfDay >= 6 && state.ecology.timeOfDay <= 18;
...
function updateEcologyStats() {
    document.getElementById('tempValue').textContent = `${state.ecology.temperature.toFixed(1)}°C`;
    document.getElementById('tempBar').style.width = `${Math.min(100, (state.ecology.temperature + 20) / 60 * 100)}%`;
    
    document.getElementById('radValue').textContent = state.ecology.radiation < 50 ? 'Low' : 'High';
    document.getElementById('radBar').style.width = `${state.ecology.radiation}%`;
    
    document.getElementById('floraValue').textContent = `${state.ecology.floraDensity.toFixed(0)}%`;
    document.getElementById('floraBar').style.width = `${state.ecology.floraDensity}%`;
    
    const activityLevel = state.ecology.faunaActivity < 30 ? 'Low' : state.ecology.faunaActivity < 70 ? 'Medium' : 'High';
    document.getElementById('faunaValue').textContent = activityLevel;
    document.getElementById('faunaBar').style.width = `${state.ecology.faunaActivity}%`;
    
    document.getElementById('healthValue').textContent = `${state.ecology.worldHealth.toFixed(0)}%`;
    document.getElementById('healthBar').style.width = `${state.ecology.worldHealth}%`;
}
